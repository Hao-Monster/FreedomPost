...
func (c *Client) SendOrder(ctx context.Context, visitor, content string) error {
	...
	if len(search.Payload) > 0 {
		...
		if x.SourceID == "" && len(search.Payload[0].ContactInboxes) > 0 {
			x.SourceID = search.Payload[0].ContactInboxes[0].SourceID
		}
	}
	if x.ID == 0 {
	...
	if x.SourceID == "" && len(x.ContactInboxes) > 0 {
		x.SourceID = x.ContactInboxes[0].SourceID
	}
	if x.SourceID == "" {
		x.SourceID = visitor
	}
	...
	return c.req(ctx, http.MethodPost, fmt.Sprintf("/api/v1/accounts/%d/conversations", c.accountID), map[string]any{"source_id": x.SourceID, "contact_id": x.ID, "inbox_id": c.inboxID, "status": "open", "message": map[string]any{"content": content, "message_type": "incoming", "private": false}}, &y)
}
